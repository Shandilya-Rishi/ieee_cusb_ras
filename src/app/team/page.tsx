...
export default function TeamPage() {
  ...
  const renderMember = (member: any) => (
    <div key={member.id} className="card" style={{ 
      ...
      width: '100%',
      maxWidth: '320px',
      margin: '0 auto' // Centers card when in flex row
    }}>
      <div style={{
        width: '120px',
        height: '120px',
    ...
    <div className="container" style={{ padding: '4rem 1.5rem' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          {CORE_TEAM.filter(m => m.role === 'Faculty Chapter Advisor').map(renderMember)}
        </div>

        {/* Tier 2: Chair & Vice Chair */}
        <div style={{ display: 'flex', justifyContent: 'center', gap: '2.5rem', flexWrap: 'wrap' }}>
          {CORE_TEAM.filter(m => m.role === 'Chair' || m.role === 'Vice Chair').map(renderMember)}
        </div>

        {/* Tier 3: Secretary */}
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          {CORE_TEAM.filter(m => m.role === 'Secretary').map(renderMember)}
        </div>

        {/* Tier 4: Rest of the Core Team */}
        <div style={{
          display: 'grid',
          gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))',
          gap: '2.5rem'
        }}>
          {CORE_TEAM.filter(m => !['Faculty Chapter Advisor', 'Chair', 'Vice Chair', 'Secretary'].includes(m.role)).map(renderMember)}
        </div>
